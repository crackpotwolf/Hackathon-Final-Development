...
export class SidebarComponent implements OnInit, AfterViewInit {
  ...
  menu: any;
  // Флаг, отвечает ли сервер на ping
  serverIsActive: boolean = false;
  ...
  pingServer() {
    ...
    this.http.get('/api/v1/account/ping', {responseType: 'text'})
      .subscribe(resp => {
        this.serverIsActive = resp == "Pong";
        runNext();
      }, err => {
  ...
  /**
   * Проверка ролей на отображение пунктов меню
   */
  isShow(itemMenu: any) {
    if (!itemMenu.role) return true;

    let tokenStr = localStorage.getItem('token');
    if (!tokenStr) return false;

    let token = this.helper.decodeToken(tokenStr)
    return token.Role && token.Role.indexOf(itemMenu.role) != -1;
  }
}
